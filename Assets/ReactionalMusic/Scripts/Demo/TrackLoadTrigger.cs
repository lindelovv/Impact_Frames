...
public class TrackLoadTrigger : MonoBehaviour
{
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag(colliderTag))
        {
            if (stopTrackOnExit)
            {
                ...
                if (Reactional.Core.ReactionalManager.Instance.tracks[selectedtrack].trackName == trackName)
                {
    ...
    public IEnumerator StartSong()
    {
        ...
        Reactional.Playback.Playlist.LoadTrack(trackName, autoplay: true);
    }
    ...
    {
        yield return new WaitForSeconds(_delayTrigger);
        Reactional.Playback.Playlist.LoadTrack(exitTrackName, autoplay: true);
        gameObject.SetActive(false);
    }
