    ...
    where C : unmanaged, IComponentData
{
    ...
    public void Execute()
    {
        ...
        for (int i = 0; i < statefulEvents.Length; i++)
        {
            ...
            var addToEntityA = (EventLookup.HasBuffer(statefulEvent.EntityA) && (!UseExcludeComponent || !EventExcludeLookup.HasComponent(statefulEvent.EntityA)));
            var addToEntityB = (EventLookup.HasBuffer(statefulEvent.EntityB) && (!UseExcludeComponent || !EventExcludeLookup.HasComponent(statefulEvent.EntityA)));

            if (addToEntityA) { EventLookup[statefulEvent.EntityA].Add(statefulEvent); }
            if (addToEntityB) { EventLookup[statefulEvent.EntityB].Add(statefulEvent); }
        }
    }
}
