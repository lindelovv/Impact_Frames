...
///    2) Add a <see cref="StatefulTriggerEventAuthoring"/> component to that entity or, if this is desired on a Character Controller:
...
public partial struct StatefulTriggerEventSystem : ISystem
{
    ...
    {
        state.Dependency = new ClearTriggerEventDynamicBufferJob().ScheduleParallel(triggerEventQuery, state.Dependency);
        events.SwapBuffers();
